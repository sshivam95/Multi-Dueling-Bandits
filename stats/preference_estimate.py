...
class PreferenceEstimate:
    ...
    We use a :class:`TrivialConfidenceRadius<stats.confidence_radius.TrivialConfidenceRadius>` for easy illustration. Note that the
    results are not accurate, you probably want to use something like
    :class:`HoeffdingConfidenceRadius<stats.confidence_radius.HoeffdingConfidenceRadius>` in practice.
    ...
    def set_pairwise_preference_score(
        ...
        for i in range(self.num_arms):
            self.skill_vector[i] += np.exp(np.inner(context_matrix[i, :], theta.T))
        new_preference_score = self.skill_vector[first_arm_index] / (
            self.skill_vector[first_arm_index] + self.skill_vector[second_arm_index]
        )
    ...
    def get_pairwise_preference_score(self, first_arm_index, second_arm_index):
        self._pairwise_preference_score[first_arm_index][
            second_arm_index
        ] = self.get_wins(first_arm_index) / (
            self.get_wins(first_arm_index) + self.get_wins(second_arm_index)
        )
    ...
    def get_confidence_radius(self, first_arm_idx: int, second_arm_idx: int) -> float:
    ...
    def get_upper_estimate(self, first_arm_index: int, second_arm_index: int) -> float:
        ...
        return min(
            ...
            + self.get_confidence_radius(first_arm_index, second_arm_index),
            1,
        )
    ...
    def sample_preference_matrix(
        ...
        The outcome is a :class:`PreferenceMatrix<stats.preference_matrix.PreferenceMatrix>` object which is initialized from a sampled
