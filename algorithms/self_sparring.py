...
class IndependentSelfSparring_v2(Algorithm):
    def __init__(
        self,
        random_state: Optional[np.random.RandomState] = None,
        joint_featured_map_mode: Optional[str] = JointFeatureMode.POLYNOMIAL.value,
        ...
        running_time: Optional[np.array] = None,
        learning_rate: float = 1,
        logger_name="IndependentSelfSparring_v2",
        logger_level=logging.INFO,
    ) -> None:
        super().__init__(
            random_state=random_state,
    ...
    def step(self) -> None:
        ...
        # self.theta_hat = np.mean(self.theta_hat, axis=0)
        self.skill_vector[self.time_step - 1] = np.mean(np.exp(np.inner(context_vector, self.theta_hat)), axis=1)
