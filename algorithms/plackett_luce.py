...
from algorithms.interfaces import AllApproximateCondorcetProducer
from algorithms.interfaces import CopelandRankingProducer
...
class PlackettLucePACItem(AllApproximateCondorcetProducer, PacAlgorithm):
    ...
    The algorithm repeatedly sorts arms with a comparison budget-constrained :class:`QuickSort<util.sorting.QuickSort>` algorithm and eliminates inferior arms.
    ...
    >>> plackett_luce.run()
    >>> comparisons = plackett_luce.feedback_mechanism.duels_conducted
    ...
    def _sort_step(self) -> List[List[int]]:
        ...
        quicksort = Quicksort(
            self._candidates,
            lambda a1, a2: determine_better_arm(self.feedback_mechanism, a1, a2),
            self.random_state,
        )
...
class PlackettLuceAMPR(CopelandRankingProducer, PacAlgorithm):
    ...
    The order of arms in different groups is known, so each group can be analyzed in isolation. The arms in each group are compared by executing the Budgeted :class:`QuickSort<util.sorting.QuickSort>` algorithm.
    ...
    >>> pl.run()
    >>> comparisons = pl.feedback_mechanism.duels_conducted
