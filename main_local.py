    ...
    for solver in solver_name:
        for name in algorithm_name:
            for subset_size in subset_size_list:
                ...
                np.save(f"Regret_results_theta_bar_feedback_correction_50//regret_{name}_{solver}_{subset_size}.npy", regrets)
                np.save(
                    f"Execution_times_results_theta_bar_feedback_correction_50//execution_time_{name}_{solver}_{subset_size}.npy",
                    execution_times,
                )
